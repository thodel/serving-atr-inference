...
def procs(monkeypatch):
    """A settable /proc: {pid: (user, age, command, ppid)}."""
    table: dict = {}
...
def test_it_totals_what_no_job_accounts_for(monkeypatch):
    ...
    assert idle["unregistered_mib"] == 27530 + 6000     # what a queued job waits for
    assert idle["orphaned_mib"] == 27530
    assert busy["unregistered_mib"] == 0
...
def test_an_unreachable_trainer_still_reports_the_cards(monkeypatch):
    ...
    assert body["cards"][1]["unregistered_mib"] == 1600
