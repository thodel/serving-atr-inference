...
```bash
cd /home/dh/.openclaw/workspace/serving-atr-inference
./scripts/make_venvs.sh
```

This creates `.venvs/troct/` and installs `requirements.txt`.

## Run

```bash
.venvs/troct/bin/python -m uvicorn trocr_svc.app:app --host 127.0.0.1 --port 8202
```

Or with systemd (after installing the unit):

```bash
sudo systemctl start atr-troct
```
...
```bash
sudo cp deploy/systemd/atr-troct.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now atr-troct
```
